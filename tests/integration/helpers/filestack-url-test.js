...
module('helper:filestack-url', function (hooks) {
  ...
  hooks.beforeEach(function () {
    ...
    this.imageUrl =
      'https://d1wtqaffaaj63z.cloudfront.net/images/NY_199_E_of_Hammertown_2014.jpg';
  });

  test('it renders a resize with options', async function (assert) {
    await render(hbs`
      {{filestack-url this.handle resize=(hash width=200 height=300 fit="crop")}}
    `);

    assert
      .dom()
      .hasText(
        `${FILESTACK_CDN}/${this.apiKey}/resize=width:200,height:300,fit:crop/${this.handle}`
      );
  });

  test('it renders a link filestack cdn without options', async function (assert) {
    await render(hbs`
      {{filestack-url this.handle}}
    `);
  ...
  test('it handles urls with missing arguments (crop is undefined here)', async function (assert) {
    await render(hbs`
      {{filestack-url this.imageUrl resize=(hash width=500 fit=this.crop)}}
    `);

    assert
      .dom()
      .hasText(
        `${FILESTACK_CDN}/${this.apiKey}/resize=width:500/"${this.imageUrl}"`
      );
  });
  ...
  test('it handles missing hash arguments', async function (assert) {
    ...
    await render(hbs`
      {{filestack-url this.handle resize=(hash width=200 height=300 fit=this.crop)}}
    `);

    assert
      .dom()
      .hasText(
        `${FILESTACK_CDN}/${this.apiKey}/resize=width:200,height:300/${this.handle}`
      );
  });

  test('it renders the original URL when options are missing & passed a URL', async function (assert) {
    await render(hbs`
      {{filestack-url this.imageUrl}}
    `);
  ...
  test('it uses customCDN config when passed an image URL', async function (assert) {
    ...
    await render(hbs`
      {{filestack-url this.imageUrl resize=(hash width=500 fit="crop")}}
    `);

    assert
      .dom()
      .hasText(
        `${customCDN}/${this.apiKey}/resize=width:500,fit:crop/"${this.imageUrl}"`
      );
  });
  ...
  test('it uses customCDN config when passed an image token', async function (assert) {
    ...
    await render(hbs`
      {{filestack-url this.handle resize=(hash width=500 fit="crop")}}
    `);

    assert
      .dom()
      .hasText(
        `${customCDN}/${this.apiKey}/resize=width:500,fit:crop/${this.handle}`
      );
  });
